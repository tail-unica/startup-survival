...
| B5 | `fix_permanenza_media_per_company` | `PermanenzaMedia` è calcolata con una `summarise` senza `group_by`, quindi è un singolo valore globale, nonostante il commento dichiari "per ciascuna CompanyID" | **MISURATO (2026-09-09): BASSO.** Con la permanenza per azienda invece che globale, `DeltaEnd` medio passa da 11,57 a 12,04 anni, le righe con dati di team da 885.142 a 892.534 (+0,84%), la somma di `Total_People` +0,91% e quella di `Total_Founders` +1,23%. Il valore globale accorcia le permanenze di circa mezzo anno: l'effetto è sistematico ma piccolo. |
...
| B4 | `fix_same_country_narm` | `any()` senza `na.rm` restituisce NA quando nessun confronto è vero e almeno uno è NA | BASSO come volume — **1.809 aziende, 1,55% di quelle con un aggregato competitor** (misurato al checkpoint B, che coincide col riferimento; la stima precedente di 1.563 era sbagliata) — **ma `Same_Country` è una feature dei modelli**. |
