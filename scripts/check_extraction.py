...
Gli intermedi in data/reference/ sono la ground truth della pipeline
...
leggendo due colonne da due file, invece di scoprirlo dopo una run intera.
    ...
    uv run python scripts/check_extraction.py "percorso/della/cartella"
"""
...
def main() -> int:
    ...
    eligible = (
        comp.with_columns(pl.col("YearFounded").cast(pl.Int64, strict=False))
        .filter(pl.col("YearFounded") > 1999)
        .height
    )
