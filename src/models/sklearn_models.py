...
class _TreeModel(Model):
    ...
    Explained on the imputed test matrix, the one the model is fitted and scored
    on. The raw frame still carries the missing values, and TreeExplainer would
    route them down the tree its own way: the SHAP values would then explain a
    prediction the model never made. The column names come from the raw frame.
    """

    def explain(self, split, shap_cfg):
        """Explain with TreeExplainer, on the imputed test matrix."""
        sample = pd.DataFrame(
            split["X_test_imp"][:N_EXPLAIN_CHEAP], columns=split["X_test"].columns
        )
