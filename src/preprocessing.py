...
def build_windowed_dataset(initialPanel, TimeWindow=7, lastYear=2024):
    """
    ...
    :param TimeWindow: Time window in years to determine the target variable (default is 7 years, as
        we want to predict if the company will reach the next growth stage within 7 years from the
        StartingAge)
    ...
    df_combined = (
        ...
        .with_columns(
            pl.min_horizontal(pl.col("StartingAge") + TimeWindow, pl.col("LastAge")).alias(
                "TargetAge"
            )
        )
        .filter(
            pl.col("StartingAge") <= 2
        )  # Age filter: consider only companies that were in Early stage at age 2 or less
    )
    ...
    df_target = df_target.with_columns(
        pl.when(
            pl.col("TargetAge") + pl.col("TimeNextStageGroup") <= pl.col("StartingAge") + TimeWindow
        )
...
def build_full_history_dataset(initialPanel, dataset_window):
    """
    This function takes the initial panel with all the companies and their growth stages at
    different ages,
    ...
    :param initialPanel: Panel with all the companies and their growth stages at different ages
    :param dataset_window: Dataset with time window, used to select the companies to include in the
        dataset without time window
...
def handle_missing_values(dataset, flag_no_time_window):
    ...
    imputing missing values in some variables with 0, imputing missing values in other variables
    with the mean of the column,
    and keeping the remaining missing values as they are to be handled by the final imputation
    strategy.
    The function also applies a feature selection based on the proportion of missing values in each
    column,
...
def preprocess_dataset(dataset, university_ranking_path, flag_no_time_window=False):
    ...
    - Turn "Gender_CEO" into a single indicator column. HQCountry and
      ...
      encoding
    - Handle missing values by dropping rows with missing values in crucial variables,
      imputing missing values in some variables with 0, imputing missing values in other
      variables with the mean of the column, and keeping the remaining missing values as
      they are to be handled by the final imputation strategy. The function also applies
      a feature selection based on the proportion of missing values in each column, and a
      row selection based on the number of missing values in each row.
    - Create the final dataset by encoding the target variable as binary (1 for "Later" and "Exit"
      (Success), 0 for "Steady")
    ...
    # columns for both the dataset with time window and the dataset without time window,
    #  and that the distribution of missing values is similar in the two datasets, so I apply the
    #  same missing values handling strategy to both datasets.
    # If you want to replicate the experiments with a different starting dataset, you should check
