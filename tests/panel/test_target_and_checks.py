...
def test_a_cumulative_column_going_backwards_is_caught():
    with pytest.raises(ValueError, match="N_Deal decreases"):
        checks.check_structure(_finished(N_Deal=[2, 1, 1]), RULES)


def test_the_counts_are_compared_with_the_expectation():
    expected = {"rows": 3, "companies": 2, "with_team": 2, "with_stage": 3}
    report = checks.check_counts(_finished(), expected)
    assert report["ok"].all()


def test_a_count_that_does_not_match_is_reported_rather_than_raised():
    wrong = {"rows": 99, "companies": 2, "with_team": 2, "with_stage": 3}
    report = checks.verify(_finished(), RULES, wrong)
    assert not report["ok"].all()
    assert report.filter(pl.col("count") == "rows")["value"].to_list() == [3]
